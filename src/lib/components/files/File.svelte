<script lang="ts">
  ...
  export let onToggle: (isTreatment: boolean) => void;
    ...
    <div class="flex flex-col space-y-2 w-full">
      ...
      <div class="flex justify-between items-center min-w-64">
        <SlideToggle
          ...
          class="mr-2"
          name="slide"
          on:change={() => onToggle(isTreatment)}
          bind:checked={isTreatment}
          ><span class={`chip ${isTreatment ? 'variant-soft-primary' : 'variant-soft-surface'}`}
            >{isTreatment ? 'Treatment' : 'Control'}</span
          ></SlideToggle
        >
