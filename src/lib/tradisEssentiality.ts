...
function calculateHistogram(data: number[], binCount: number): { bins: number[], counts: number[] } {
...
export function tradisEssentiality(geneInsertResults: GeneInsertResult[]): GeneEssentialityResult {    
    ...
    const maxIndex = densities.slice(10).reduce((maxIdx, val, idx) => val > densities[maxIdx] ? idx : maxIdx, 0) + 10;
    ...
    const minimaIndex = loessFit.reduce((minIdx, val, idx) => val < loessFit[minIdx] ? idx : minIdx, 0);
    ...
    const ambigGenes = geneInsertResults.filter(gene => gene.insIndex >= essen && gene.insIndex < ambig);
