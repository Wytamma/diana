...
function createInsertStore() {
  ...
  return {
    ...
    setIsTreatment: (name: string, value: boolean) =>
      update((store) => {
...
async function parseTextInChunks(
  text: string,
  chunkSize: number = 50000
): Promise<InsertCountData> {
  ...
  for (let i = 0; i < lines.length; i += chunkSize) {
    ...
    chunk.forEach((line) => {
      if (line.startsWith('variableStep')) {
        ...
        totals[currentChrom].set(
          position,
          (totals[currentChrom].get(position) || 0) + Math.abs(total)
        );
      } else {
        // Skip other lines
      }
