 ...
 */
async function generateFilteredUrls(
  filteredFeatures: Feature[]
): Promise<{ url: string | undefined; indexUrl: string | undefined }> {
  ...
  const gffFileText = filteredFeatures
    .map(
      (feature) =>
...
export function createAnnotationStore() {
  ...
  return {
    ...
    load: async (filename: string, text: string) => {
      ...
      for (const line of lines) {
        ...
        if (!line.startsWith('#') && line.trim() !== '') {
          ...
          const attributes: { [key: string]: string | undefined } = Object.fromEntries(
            attributeString.split(';').map((attr) => attr.split('='))
          );
    ...
    setFilteredFeatures: async (featureIds: number[]) => {
      ...
      update((store) => {
        store.filteredFeatures = store.features.filter((feature) =>
          featureIds.includes(feature._id)
        );
